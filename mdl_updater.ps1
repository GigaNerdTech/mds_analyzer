...
if (Get-Module -ListAvailable -Name VMware.VimAutomation.Core) {
    ...
    if (Get-PSRepository | Where-Object { $_ -match "mhs-nuget" }) {
        Install-Module -Name VMware.PowerCLI -Repository "mhs-nuget" -AllowClobber
        Import-Module VMware.VimAutomation.Core
    }
    else {
        Register-PSRepository -Name 'mhs-nuget' -SourceLocation 'https://nuget.mhs.int/nuget' -InstallationPolicy Trusted
        Install-Module -Name VMware.PowerCLI -Repository "mhs-nuget" -AllowClobber
        Import-Module VMware.VimAutomation.Core
    }
}
...
# Start Transcript
$TranscriptFile = "\\funzone\team\POSH\Powershell\PSLogs\MDLUpdater_$(get-date -f MMddyyyyHHmmss).txt"
$start_time = Get-Date
Start-Transcript -Path $TranscriptFile
$vhosts = @("vcenter-dal.mhd.com","vcenter-man.mhd.com","mhsvc01","mhsvc02")

$server_list_file = "\\firework\Infosys\IT_Operations\Application_Identification\Combined Server List_$(get-date -f MMddyyyyHHmmss).xlsx"

Copy-Item "\\firework\Infosys\IT_Operations\Application_Identification\Combined Server List.xlsx" -Destination $server_list_file -Force
...
foreach ($vm in ($vms_not_in_mdl | Sort)) {
    ...
    $worksheet1.Cells.Item($new_rows,9) = "MHS"
...
foreach ($vcenter in $vhosts) {
    $vp_inventory[$vcenter] | ForEach-Object {
        ...
        $vp = $_
        $vp_name = $vp.Name
        if (($physical_servers | ForEach { "$($_)" }) -notcontains ($vp.Name -replace "\.mhs\.int|\.mhd\.com","").ToUpper()) {
        Write-Host "Adding vCenter physical server $vp_name..."
        $worksheet1.Cells.Item($new_rows,1) = ($vp.Name -replace "\.mhs\.int|\.mhd\.com","").ToUpper()
        ...
        $worksheet1.Cells.Item($new_rows,13) = "Edwards, Joey"
        $worksheet1.Cells.Item($new_rows,22) = "Added by MDL Script!"
        $worksheet1.Range($worksheet1.Cells($new_rows,1),$worksheet1.Cells($new_rows,22)).Interior.ColorIndex = 4
        $new_rows++
        $vp_count++
...
for ($i=2; $i -le $row_count; $i++) {
   ...
   # Mark MDL spreadsheet with VMs not in vCenter
    ...
    # Campus names: MDMC->Dallas, MMMC->Mansfield
    if ($worksheet1.Cells.Item($i,7).Text -match "MMMC") {
        $worksheet1.Cells.Item($i,7) = "Mansfield"
    }
    if ($worksheet1.Cells.Item($i,7).Text -match "MDMC") {
        $worksheet1.Cells.Item($i,7) = "Dallas"
    }
...
$email_list=@("joshuawoleben@mhd.com","michaelfink@mhd.com",$email_user)
...
$MailMessage = @{
    To = $email_list
    From = "MDLReport<Donotreply@mhd.com>"
    Subject = $subject
    Body = ($body -join "<br/>")
...
<#    if ($vm_name -match "EPC-NPD|EPC-REL") {
        $worksheet1.Cells.Item($new_rows,2) = "4"
        $worksheet1.Cells.Item($new_rows,5) = "Test"
        $worksheet1.Cells.Item($new_rows,12) = "Server Team"
        $worksheet1.Cells.Item($new_rows,13) = "Edwards, Joey"
    }
    if ($vm_name -match "EPC-PRD") {
        $worksheet1.Cells.Item($new_rows,2) = "1"
        $worksheet1.Cells.Item($new_rows,5) = "Prod"
        $worksheet1.Cells.Item($new_rows,12) = "Server Team"
        $worksheet1.Cells.Item($new_rows,13) = "Edwards, Joey"
    }
    if ($vm_name -match "EPC-...-HS") {
        $worksheet1.Cells.Item($new_rows,3) = "Epic Hyperspace Citrix"
        $worksheet1.Cells.Item($new_rows,4) = "Epic Hyperspace Citrix"
    }
    if ($vm_name -match "EPC-...-IC") {
        $worksheet1.Cells.Item($new_rows,3) = "Epic Interconnect"
        $worksheet1.Cells.Item($new_rows,4) = "Epic Interconnect"
    }
    if ($vm_name -match "EPC-...-HW") {
        $worksheet1.Cells.Item($new_rows,3) = "Epic Hyperspace Web"
        $worksheet1.Cells.Item($new_rows,4) = "Epic Hyperspace Web"
    }
    if ($vm_name -match "EPC-...-EPS") {
        $worksheet1.Cells.Item($new_rows,3) =  "Epic Print Server"
        $worksheet1.Cells.Item($new_rows,4) = "Epic Print Server"
    }
    if ($vm_name -match "aiasv") {
        $worksheet1.Cells.Item($new_rows,2) = "5"
        $worksheet1.Cells.Item($new_rows,3) = "Citrix"
        $worksheet1.Cells.Item($new_rows,4) = "XenDesktop VDI Application Group"
        $worksheet1.Cells.Item($new_rows,5) = "Test"

    }
    if ($vm_name -match "sccm") {
        $worksheet1.Cells.Item($new_rows,2) = "5"
        $worksheet1.Cells.Item($new_rows,3) = "SCCM"
        $worksheet1.Cells.Item($new_rows,5) = "Test"
        $worksheet1.Cells.Item($new_rows,13) = "Edwards, Joey"
    }
    if ($vm_name -match "asvr|atoc") {
        $worksheet1.Cells.Item($new_rows,2) = "5"
        $worksheet1.Cells.Item($new_rows,3) = "Citrix"
        $worksheet1.Cells.Item($new_rows,4) = "XenDesktop VDI"
        $worksheet1.Cells.Item($new_rows,5) = "Server Team"
        $worksheet1.Cells.Item($new_rows,13) = "Edwards, Joey"
    }
    if ($vm_name -match "blaw") {
        $worksheet1.Cells.Item($new_rows,2) = "3"
        $worksheet1.Cells.Item($new_rows,3) = "Lawson"
        $worksheet1.Cells.Item($new_rows,4) = "Modem"
        $worksheet1.Cells.Item($new_rows,12) = "Lawson"
        $worksheet1.Cells.Item($new_rows,13) = "Ingram, William"
    }
    if ($vm_name -match "CTXSRV") {
        $worksheet1.Cells.Item($new_rows,3) = "Citrix"
    }#>